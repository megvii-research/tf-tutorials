...
def train(args):
    log_dir = os.path.join(args.output_dir, "M{}_L{}_W{}_F{}".format(
        args.model, args.lr, args.weight_tv, "-".join(args.feature)
    ))
    ...
    pre_noise = np.random.uniform(low=-3, high=3, size=img.shape).astype(np.float32)
    ...
    for epoch in range(args.epoch):
        ...
        for item in args.feature:
            img_output = extract_feature(model, img_tensor, item, transform_dict)
            noise_output = extract_feature(model, noise_tensor, item, transform_dict)
        ...
        if epoch % args.save_interval == 0:
            noise_np = noise_tensor.data.cpu().squeeze(0).permute(1, 2, 0).contiguous().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epoch', default=4000)
